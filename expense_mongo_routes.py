...
"""
expense_mongo_routes.py  —  MongoDB save/sync for ExpenseTracker
Supports: expenses + budget (accounts) data
...
class SaveRequest(BaseModel):
    ...
    password: str
    expenses: List[ExpenseItem]
    budget: Optional[Dict[str, Any]] = None   # ← NEW: full budget object
...
async def save_expenses(req: SaveRequest):
    """
    Register (first time) or login + save expenses + budget to cloud.
    Merges expenses by id — local list wins for overlapping ids.
    Budget is stored as a single document per user (fully replaced on each save).
    """
    db = get_db()
    uname = req.username.strip().lower()
    ...
    # ── Save budget/accounts (upsert — one doc per user) ──
    ...
    return {
        "success": True,
        "saved": len(merged),
        "budget_saved": req.budget is not None,
        "message": (
            f"{'Registered and saved' if user is None else 'Saved'} "
            f"{len(merged)} expenses"
            f"{' + account data' if req.budget is not None else ''}."
        ),
    }
